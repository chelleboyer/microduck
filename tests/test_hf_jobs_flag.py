...
def test_train_on_path_is_a_mjlab_trainer():
    ...
    # Whole file, not the first 8 KB: on POSIX the console script is a text
    # file whose shebang+import sit at the top, but on Windows it is a ~41 KB
    # .exe launcher with the Python shim appended at the END (measured: byte
    # 40574). Reading only the head passes on Linux and fails on Windows for a
    # purely cosmetic reason.
    head = Path(exe).read_bytes()
