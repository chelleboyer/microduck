...
# POSIX-only terminal machinery, used solely by the interactive keyboard driver.
# Imported defensively so the MODULE stays importable on Windows: the BAM/CPU
# rehearsal helpers here are worth unit-testing on any platform, and an
# unconditional import made `import infer_policy` a hard ImportError on Windows
# — which is why this repo's own notes record the script as simply unrunnable
# there. The keyboard path checks HAS_TTY and degrades instead of crashing.
try:
    import select
    import termios
    import tty
...
class TerminalInput:
    ...
    def __init__(self):
        ...
        # HAS_TTY: on Windows there is no termios/tty, so keyboard control is
        # unavailable even when stdin IS a tty. Same degradation path either way.
        self.enabled = HAS_TTY and sys.stdin.isatty()
